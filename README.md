...
* [Fruits de ma formation](#fruits-de-ma-formation)
...
## Fruits de ma formation

Projets réalisés en suivant les tutos de cette liste.

### Mini-shell en C

- **Tuto suivi :** [Stephen Brennan — *Write a Shell in C*](https://brennan.io/2015/01/16/write-a-shell-in-c/)
- **Mon code :** [dossier `formation/mini-shell/`](formation/mini-shell/) *(remplace par ton dépôt GitHub si le code est ailleurs, ex. `https://github.com/tonpseudo/mon-shell`)*
- **Notes :** compris le cycle `fork` → `exec` pour lancer une commande, le parsing ligne par ligne avec `strtok`, et pourquoi il faut absolument gérer `SIGCHLD` pour éviter les zombies quand un enfant se termine en arrière-plan. La partie la plus pénible : raccorder correctement les pipes pour `cmd1 | cmd2` sans bloquer le shell.

![Capture du terminal](formation/mini-shell/capture.png)

### Ray tracer (week-end)

- **Tuto suivi :** [Peter Shirley — *Ray Tracing in One Weekend*](https://raytracing.github.io/books/RayTracingInOneWeekend.html)
- **Mon code :** [dossier `formation/ray-tracer/`](formation/ray-tracer/)
- **Notes :** premier rendu avec sphères et matériaux diffuse / métal ; compris l’idée de lancer un rayon par pixel et d’accumuler la couleur selon les rebonds. Beaucoup de temps passé à débugger les normales et l’orientation de la caméra.

![Rendu final](formation/ray-tracer/rendu.png)

### …

Les images doivent être dans le dépôt (dossier [`formation/`](formation/)), pas seulement sur ton disque.
